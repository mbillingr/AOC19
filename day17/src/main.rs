...
fn main() {
    ...
    let output: String = String::from_utf8(output.into_iter().map(|o| o as u8).collect::<Vec<u8>>()).unwrap();
    println!("{}", output);

    let grid: Vec<Vec<u8>> = output.lines().map(str::bytes).map(Iterator::collect).collect();
    ...
    for i in 1..height-1 {
        for j in 1..width-1 {
            if grid[i][j] == b'#' && grid[i+1][j] == b'#' && grid[i-1][j] == b'#' && grid[i][j+1] == b'#' && grid[i][j-1] == b'#' {
                alignment_sum += i * j;
            }
        }
    }
    ...
    for i in 0..height {
        for j in 0..width {
            if grid[i][j] == b'^' {
                start_pos = Pos{x:j as i64, y:i as i64};
            }
        }
    }

    let prog2: Vec<_> = std::iter::once(2).chain(INPUT.iter().skip(1).copied()).collect();
...
fn find_partition(path: &Vec<Command>) {
    for la in (2..10) {
        let a_slice = &path[0..la];
        let a_str = if let Some(s) = stringify(a_slice) {
            s
        } else {
            continue
        };
        ...
        let mut main = vec!['A'];
        let mut rest = &path[la..];

        while rest.starts_with(a_slice) {
            main.push('A');
            rest = &rest[la..];
        }
